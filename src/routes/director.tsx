  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Director — Prathamesh Sudam Bhase" },
      {
        ...
        content:
          "Director of Operations at Precise Industries, Pune. Get in touch directly.",
      },
    ],
  }),
  component: Director,
...
function Director() {
  ...
  const directorName = settings["director_name"] || COMPANY.contactPerson.name;
  const directorRole = settings["director_role"] || COMPANY.contactPerson.role;
  ...
  const socials = [
    ...
    { label: "Facebook", icon: Facebook, href: settings["facebook_url"] },
    {
      ...
      icon: MessageCircle,
      href:
        settings["whatsapp_url"] || `https://wa.me/${phone.replace(/\D/g, "")}`,
    },
    {
      label: "Gmail",
      icon: Mail,
      href: settings["email_url"] || `mailto:${settings["email_address"]}`,
    },
  ];
        ...
        <Reveal>
          <div className="rounded-3xl border border-border bg-surface/60 p-8 text-center backdrop-blur-xl md:p-12">
            ...
            <p className="mt-9 text-sm leading-relaxed text-muted-foreground">
              Leading day-to-day operations at {settings["company_name"]},{" "}
              {COMPANY.city} — precision machining, grinding, die and mould
              components, and hard chrome plating.
            </p>
          </div>
        </Reveal>
      </section>
